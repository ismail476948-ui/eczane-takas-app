...
function AdminDashboard() {
  ...
  // --- YENİ EKLENEN SÜPÜRGE FONKSİYONU ---
  const handleDatabaseCleanup = async () => {
    const confirmMessage = "Sistemdeki tüm 'Silinmiş Eczane' kayıtları (takaslar, ödemeler, sahipsiz ilaçlar) kalıcı olarak silinecek.\n\nBu işlem geri alınamaz. Emin misiniz?";
    
    if (!window.confirm(confirmMessage)) return;

    try {
        const res = await axios.post('/api/admin/cleanup', {}, {
            headers: { 'x-auth-token': token }
        });
        
        const { details } = res.data;
        alert(`🧹 Temizlik Tamamlandı!\n\nSilinen Takas: ${details.ordersRemoved}\nSilinen Ödeme: ${details.paymentsRemoved}\nSilinen İlaç: ${details.medicinesRemoved}`);
        
        // İstatistikleri ve sayfayı güncelle
        fetchStats(); 
    } catch (error) {
